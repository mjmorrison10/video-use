...
def mix(cid, track, out_path, gain=None, level=1.0, compress=False, full_length=True):
    """Lay a steady, normalized music bed under the clip.

    full_length=True (TRW default): music plays the ENTIRE clip, with only a short
    end-fade so it doesn't click — the music runs under the final line too.
    full_length=False: the older style where the bed CUTS ~0.6s before the last
    words so the punch lands in silence (`compute_climax`).
    """
    clip = ED / "out" / f"clip_{cid}.mp4"
    climax, total = compute_climax(cid)
    music_dur = total if full_length else climax
    seg_start, seg_rms = find_best_segment(track, music_dur)
    ...
    fade_out_st = max(0.1, (total - 0.35) if full_length else (climax - 0.5))
    fade_out_d = 0.35 if full_length else 0.5
    comp = "acompressor=threshold=0.12:ratio=4:attack=20:release=250," if compress else ""
    af = (f"[1:a]{comp}afade=t=in:st=0:d=1.2,afade=t=out:st={fade_out_st:.2f}:d={fade_out_d},"
          f"volume={g:.3f}[m];"
          f"[0:a][m]amix=inputs=2:normalize=0:duration=first[mix];"
          f"[mix]alimiter=limit=0.97[a]")
    cmd = ["ffmpeg", "-y", "-i", str(clip), "-ss", f"{seg_start:.2f}", "-t", f"{music_dur:.2f}",
           "-i", str(track), "-filter_complex", af, "-map", "0:v", "-map", "[a]",
           "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", str(out_path)]
    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
    tag = "full" if full_length else f"cut@{climax}s"
    print(f"{cid}: music {tag}/{total}s  seg@{seg_start}s  -> {out_path}")
